...
def _run(embedding_name: str):
    embedding = load_embedding(Path.cwd().joinpath(f"data/20_embeddings/{embedding_name}").__str__())
    result: List[Tuple[int, float]] = embedding.search("Ablehnung Prozesskostenhilfe", limit=3)
    _print(result)


def _print(result):
    for i, pred in result:
        with open(f"/Users/philipp/git/cctxtai/data/10_transformed/{i}.txt", "r") as file:
